...
module "networking" {
  ...
  ddos_id                           = can(each.value.ddos_protection_plan_id) ? each.value.ddos_protection_plan_id : try(local.combined_objects_ddos_services[try(each.value.ddos_services_lz_key, local.client_config.landingzone_key)][try(each.value.ddos_services_key, each.value.ddos_services_key)].id, "")
